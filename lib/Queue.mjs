/*
    ...
    - Fila é uma lista linear de acesso restrito, que permite apenas as operações
      de inserção (enqueue) e retirada (dequeue), a primeira ocorrendo no final da estrutura
      e a outra no início da estrutura.
    - Como consequência, a fila funciona pelo princípio FIFO (First In, First Out -
      último a entrar, primeiro a sair).
    - A principal aplicação das filas são tarefas de processamento de itens em ordem de chegada.
*/
...
export class Queue {
    ...
    // Inserção da fila (ocorre no final)
    enqueue(val) {
        this.#data.push(val)
    }
